...
set_repo_data () {
  ...
  ifFunc 'custom_source_versions' \
    && eval_param_ifn 'SOURCE_VERSIONS' "custom_source_versions \"${SOURCE_REPO_TAGS}\"" \
    || eval_param_ifn 'SOURCE_VERSIONS' "echo \"${SOURCE_REPO_TAGS}\" | xargs -n1 | grep -Eo \"${VERSION_REGEX//\\/\\\\}\" | sort -uV | xargs"

  ifFunc 'custom_source_major_versions' \
    && eval_param_ifn 'SOURCE_MAJOR_VERSIONS' "custom_source_major_versions \"${SOURCE_VERSIONS}\"" \
    || eval_param_ifn 'SOURCE_MAJOR_VERSIONS' "parse_version_major \"${SOURCE_VERSIONS}\""
  eval_param_ifn 'SOURCE_MINOR_VERSIONS' "parse_version_minor \"${SOURCE_VERSIONS}\""
}
...
get_target_versions () {
  case "${TARGET_VERSION_TYPE}" in
    ...
    minor|major)
      ...
      for ver in ${versions}; do
        target_versions="${target_versions}$(echo "${SOURCE_VERSIONS}" | xargs -n1 | grep -oP "(^${ver}\..*)" | sort -uV | tail -n1) "
      done
      ;;
    custom|*)
      # default to the full minor version list if there's no match
      target_versions="${REPO_MINOR_VERSIONS}"
      ;;
  esac
  ...
  [ ! -z "${MINIMUM_VERSION}" ]  \
    ...
    || min_ver="$(echo "${REPO_MINOR_VERSIONS}" | xargs -n1 | tail -n +1 | head -n1)"
...
check_updates () {
  ...
  for ver in $(echo "${vers}" | xargs -n1 | sort -uV); do
    ...
    [ ! -n "${repo_latest}" ] \
      && repo_latest="$(echo "${REPO_VERSIONS}" | grep -Po "(${ver}[\.\d]*|$)" | sort -uV | tail -n1)"
    [ ! -n "${source_latest}" ] \
      && source_latest="$(echo "${SOURCE_VERSIONS}" | grep -Po "(${ver}[\.\d]*|$)" | sort -uV | tail -n1)"
